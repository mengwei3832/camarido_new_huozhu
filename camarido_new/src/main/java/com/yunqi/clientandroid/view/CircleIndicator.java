...
public class CircleIndicator extends View {
	...
	private final int DEFAULT_INDICATOR_LAYOUT_GRAVITY = Gravity.CENTER
			.ordinal();
	...
	private void init(Context context, AttributeSet attrs) {
		tabItems = new ArrayList<ShapeHolder>();
	...
	/**
	 * trigger to redraw the indicator when the ViewPager's selected item
	 * changed!
	 * 
	 * @param position
	...
	private void trigger(int position, float positionOffset) {
		...
		Log.e("CircleIndicator", "onPageScrolled()" + position + ":"
				+ positionOffset);
		requestLayout();
		invalidate();
	}
	...
	private void layoutTabItems(final int containerWidth,
		...
		if (this.viewPager.getAdapter().getCount() > 0) {
			...
			for (int i = 0; i < tabItems.size(); i++) {
				...
				float x = startPosition
						+ (mIndicatorMargin + mIndicatorRadius * 2) * i;
				item.setX(x);
			}
		}
	...
	private float startDrawPosition(final int containerWidth) {
		if (mIndicatorLayoutGravity == Gravity.LEFT)
			return 0;
		float tabItemsLength = tabItems.size()
